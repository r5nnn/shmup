...
class CollisionDetector:
    ...
    def deregister(self, group: pygame.sprite.Group, group_key: pygame.sprite.Group) -> None:
        if group in self.collision_checks:
            self.collision_checks[group] = [_collision_group for _collision_group in self.collision_checks[group] if _collision_group != group_key]
        elif group_key in self.collision_checks:
            self.collision_checks[group_key] = [_group for _group in self.collision_checks[group_key] if _group != group]

    # noinspection PyTypeChecker
    def update(self):
        for group_key in self.collision_checks:
            for group in self.collision_checks[group_key]:
                if sprite_dict := pygame.sprite.groupcollide(group_key, group, False, False, pygame.sprite.collide_mask):
