...
class Entity(pygame.sprite.Sprite):
    ...
    def __init__(self, game: 'Game', img_dir: str = None):
        ...
        super().__init__()
        self.game = game
        self.img_dir = img_dir if img_dir is not None else \
            os.path.join(self.game.textures_dir, 'null')
        self.spritesheet = SpritesheetMaker(self.img_dir)
        self.rect = None
        self.hp = None
        self.atk = None
...
class Enemy(Entity):
    ...
    def update(self) -> None:
        ...
        if self.hp <= 0:
            self.kill()
            self.die_sfx.play_audio('enemy_die', override=True)
