...
class Shader:
    ...
    @staticmethod
    def create_vertfrag_shader(ctx: moderngl.Context, vertex_filepath: str,
                               fragment_filepath: str) -> moderngl.Program:
        """
        ...
        shader = ctx.program(vertex_shader=vertex_src,
    ...
    def __init__(self, vertex_path: str, fragment_path: str,
        ...
        self.shader_data = {}
        self.shader = Shader.create_vertfrag_shader(self.ctx, vertex_path,
                                                    fragment_path)
        self.render_rect = screen_rect.ScreenRect(
            self.target_surface.get_size(), self.target_surface.get_size(),
            (0, 0), self.ctx, self.shader)

        self.screen_texture = texture.Texture(
            pygame.Surface(self.target_surface.get_size()), self.ctx)
        self.framebuffer = self.ctx.simple_framebuffer(
            size=self.target_surface.get_size(), components=4)
        self.scope = self.ctx.scope(self.framebuffer)

        self.window_size = pygame.display.get_surface().get_size()

    def clear(self,
              color: typing.Union[pygame.Color, typing.Tuple[int]]) -> None:
        """
    ...
    def render_direct(self, rect: pygame.Rect, update_surface: bool = True,
        ...
        if autoscale:
            size = (
            self.target_surface.get_width(), self.target_surface.get_height())
        else:
            size = self.window_size

        rect = screen_rect.ScreenRect.pygame_rect_to_screen_rect(rect,
                                                                 self.target_surface,
                                                                 size)

        # self.__upload_uniforms()
        self.render_rect = screen_rect.ScreenRect((rect.w, rect.h), size,
                                                  (rect.x, rect.y), self.ctx,
                                                  self.shader)
    ...
    def render(self, update_surface: bool = True) -> pygame.Surface:
        ...
        with self.scope:
            ...
            self.render_rect.vao.render()
            surf = pygame.image.frombuffer(self.framebuffer.read(),
...
class ComputeShader:
    ...
    @staticmethod
    def create_compute_shader(ctx: moderngl.Context,
                              compute_shader_path: str) -> moderngl.ComputeShader:
        """
...
class DefaultScreenShader(Shader):
    ...
    def render(self) -> None:
        ...
        super().render_direct(pygame.Rect(0, 0, self.target_surface.get_width(),
                                          self.target_surface.get_height()),
                              autoscale=True)